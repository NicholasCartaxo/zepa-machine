...
const (
	bufferSize       = 64 * 1024
	minKernelSize    = 8 * 1024 * 1024
	kernelMaxMemory  = 8 * 1024 * 1024
	pcbBase          = 0x102C
	pcbSize          = 84
	flagsOffset      = 80
	partitionNumAddr = 0x1014
...
func bootKernel(cfg *Config) (*machine.Machine, uint32, error) {
	...
	if cfg.PartitionSize%4 != 0 {
		return nil, 0, fmt.Errorf("partition_size must be a multiple of 4, got %d", cfg.PartitionSize)
	}
	if cfg.MemorySize < minKernelSize+bufferSize+cfg.PartitionSize {
		return nil, 0, fmt.Errorf("memory_size must be at least %d + %d + partition_size, got %d", minKernelSize, bufferSize, cfg.MemorySize)
	}
	...
	binary.LittleEndian.PutUint32(m.GetMemory()[0x1000:0x1004], uint32(cfg.PartitionSize))
	binary.LittleEndian.PutUint32(m.GetMemory()[0x1004:0x1008], uint32(cfg.TimeSlice))
	binary.LittleEndian.PutUint32(m.GetMemory()[0x100C:0x1010], uint32(bufferSize))

	go m.Boot()

	deadline := time.Now().Add(bootTimeout)
	for readUint32(m.GetMemory(), partitionNumAddr) == 0 {
	...
	userMemory := uint32(cfg.MemorySize) - kernelMaxMemory - bufferSize
	partitionNumber := userMemory / uint32(cfg.PartitionSize)
	if partitionNumber == 0 {
		return nil, 0, errors.New("no user partitions available")
	}

	return m, partitionNumber, nil
}
